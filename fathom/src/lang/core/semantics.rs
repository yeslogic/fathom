...
use crate::lang::core::{
    FieldDefinition, Globals, Item, ItemData, LocalLevel, LocalSize, Locals, Primitive, Sort, Term,
    TermData,
};
    ...
    match &term.data {
        ...
        TermData::Item(item_name) => match items.get(item_name.as_str()) {
            ...
            Some(item) => match &item.data {
                ItemData::Constant(constant) => eval(globals, items, locals, &constant.term),
