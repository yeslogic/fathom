...
pub enum ParseExpr {
    ...
    /// (x : p1) (y : p2) (z : p3) => expr
    /// ```
    Sequence(Vec<Named<OwnedIdent, RcParseExpr>>, RcExpr),
...
pub enum Expr {
    ...
    Proj(RcExpr, OwnedIdent),
    Intro(Path, OwnedIdent, RcExpr),
    Subscript(RcExpr, RcExpr),
    Cast(RcExpr, RcType),
    Lam(Vec<Named<OwnedIdent, RcType>>, RcExpr),
