...
use semantics::{Definition, TcEnv};
...
pub fn match_value(
    ...
    match (&*pattern.inner, &*value.inner) {
        ...
        (&Pattern::Var(Embed(Var::Free(ref free_var))), _) => match env
            .get_definition(free_var)
            .and_then(|definition| match definition {
                Definition::Alias(ref term) => Some(nf_term(env, term)),
