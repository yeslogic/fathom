...
use name::{Name, Named};
use source::Span;
use structural::ast::{self, Field, Var, DEFAULT_PRETTY_WIDTH};
...
impl<N: Name> Expr<N> {
    ...
    pub fn to_doc(&self) -> Doc<BoxDoc> {
        match *self {
            Expr::Var(_, ref var) => Doc::text(var.name().to_string()),
...
impl<N: Name> fmt::Display for Expr<N> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.to_doc()
...
impl<N: Name> Type<N> {
    ...
    pub fn to_doc(&self) -> Doc<BoxDoc> {
        match *self {
            Type::Var(ref var) => Doc::text(var.name().to_string()),
